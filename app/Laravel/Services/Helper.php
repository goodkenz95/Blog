...
class Helper {
	...
	public static function send_sms($contact_number, $msg_body = "Hello!"){
		try{
			...
			$response = $curl->request("POST",env("SMS_SEND_URL",'http://beta.semaphore.co/api/v4/messages'),
						[
						'headers' => [
						        // 'Authorization' => 'AccessKey '.env("SMS_KEY","test_cHOHiy0MFuuohTEMzadsg0p4t"),
	...
	public static function nice_number($number,$sepator = ",",$precision="0"){
		return number_format($number,$precision,"",$sepator);
	}


	public static function amount($number,$sepator = ",",$precision="2"){
		return number_format($number,$precision,".",$sepator);
	}
	...
	public static function online_time_passed($time){
		$current_date = Carbon::now();
		$secsago = strtotime($current_date) - strtotime($time);
		$nice_date = 1;
		if ($secsago < 60):
			if($secsago < 30){ return "Active";}
		...
		elseif ($secsago < 432000):
		   ...
		   $nice_date = 0;
		   $period = date("d/M/Y",strtotime($time));

		   if(Carbon::now()->format("Y") == date("Y",strtotime($time))){
		   		$period = date("j M",strtotime($time));
		   }
		endif;
		if($nice_date == 1)
			return $period." ago";
	...
	public static function chat_time_passed($time){
		$date = Carbon::parse($time);
	...
	public static function mround($number, $precision=0) { 
	    ...
	    if($diffCeil >= $diffFloor) return self::amount($floor,"",1); 
	    else return self::amount($ceil,"",1); 
	} 
