            ...
            <tbody>
              ...
              <tr>
                <td class="cell-detail"> 
                  ...
                  <span class="cell-detail-description">{{"ID ".str_pad($category->id, 4, "0", STR_PAD_LEFT)}}</span>
                </td>
                <td class="cell-detail text-center"> 
                  <span class="text-{{Helper::status_badge($category->status)}}"><strong>{{Str::title($category->status)}}</strong></span>
                </td>
                <td class="cell-detail">
                  <span>{{Helper::date_only($category->created_at)}}</span>
                  <span class="cell-detail-description">{{Helper::date_format($category->created_at,"h:i A")}}</span>
                </td>
                <td class="actions">
                    <div class="btn-group btn-hspace">
                      ...
                      <ul role="menu" class="dropdown-menu">
                        <li><a href="{{route('system.category.edit',[$category->id])}}">Edit Record</a></li>
                        <li class="divider"></li>
                        <li><a class="action-delete" href="#" data-url="{{route('system.category.destroy',[$category->id])}}" data-toggle="modal" data-target="#confirm-delete" title="Remove Record">Remove Record</a></li>
                      </ul>
                    </div>
                </td>
              </tr>
              @empty
              <td colspan="4" class="text-left"><i>No record found yet.</i> <a href="{{route('system.category.create')}}"><strong>Click here</strong></a> to create one.</td>
              @endforelse
            </tbody>
          </table>
